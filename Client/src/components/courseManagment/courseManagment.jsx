...
const ManageCoursePage = () => {
  ...
  useEffect(() => {
    if (courseFindSuccess) {
      setSelectedCourse(
        courseFindSuccess?.data.find((item) => item._id === id)
      );
    }
  }, [courseFindSuccess]);
  return (
            ...
            <List>
              {manageCourseItems.map((item, index) => (
                <ListItem
                  key={item.name}
                  onClick={() =>
                    setSelectedCourseItem(manageCourseItems[index])
                  }
                  sx={{
                    border:
                      selectedCourseItem?.name === item.name
                        ? "2px solid #6255FA"
                        : "2px solid #000", // Purple border when selected
                    borderRadius: "6px",
                    display: "flex",
                    alignItems: "center",
                    ...
                    backgroundColor:
                      selectedCourseItem?.name === item.name
                        ? "#6255FA"
                        : "transparent", // Purple background if selected
                    "& .MuiListItemIcon-root": {
                      color:
                        selectedCourseItem?.name === item.name ? "#fff" : "#000" // Icon color: white if selected, black otherwise
                    },
                    "& .MuiListItemText-root": {
                      color:
                        selectedCourseItem?.name === item.name ? "#fff" : "#000" // Text color: white if selected, black otherwise
                    },
                    "&:hover": {
